...
<div class="mx-auto max-w-4xl">
    ...
    <?php else: ?>
        <div class="mt-8 overflow-hidden rounded-2xl border border-white/10 bg-[#151e30]">
            <?php foreach ($itensCarrinho as $chaveItem => $quantidade):
                $partesItem = explode(':', (string) $chaveItem, 2);
                $produto = buscarProdutoPorId((int) $partesItem[0]);
                if (!$produto) continue;
                $variacao = isset($partesItem[1]) ? ($produto['variacoes'][(int) $partesItem[1]] ?? null) : null;
                $imagemItem = $variacao['imagem'] ?? $produto['imagem'];
                $subtotal = $produto['preco'] * $quantidade; $total += $subtotal;
                $estoqueAtualItem = (int) ($produto['estoque'] ?? 0);
            ?>
                <article class="flex flex-col gap-5 border-b border-white/10 p-5 last:border-0 sm:flex-row sm:items-center"><img src="<?php echo htmlspecialchars($imagemItem); ?>" alt="" class="h-24 w-full rounded-lg object-cover sm:w-32"><div class="flex-1"><h2 class="font-extrabold"><?php echo htmlspecialchars($produto['nome']); ?></h2><?php if ($variacao): ?><p class="mt-1 text-sm font-bold text-blue-300">Variação: <?php echo htmlspecialchars($variacao['nome']); ?></p><?php endif; ?><p class="mt-1 text-sm text-slate-400">Quantidade: <?php echo (int) $quantidade; ?></p><?php if ($estoqueAtualItem < $quantidade): ?><p class="mt-1 text-xs font-bold text-red-400">Estoque insuficiente (disponível: <?php echo $estoqueAtualItem; ?>). Ajuste a quantidade antes de finalizar.</p><?php endif; ?></div><strong class="text-lg">R$ <?php echo number_format($subtotal, 2, ',', '.'); ?></strong><form method="post" action="index.php"><?php echo csrfCampo(); ?><input type="hidden" name="acao" value="remover"><input type="hidden" name="item" value="<?php echo htmlspecialchars((string) $chaveItem); ?>"><button type="submit" class="rounded-md border border-red-400/40 px-3 py-2 text-sm font-bold text-red-300 transition hover:bg-red-500 hover:text-white" aria-label="Remover <?php echo htmlspecialchars($produto['nome']); ?>"><i class="ph ph-trash"></i></button></form></article>
            <?php endforeach; ?>
            <div class="flex flex-col gap-5 bg-[#101725] p-6 sm:flex-row sm:items-center sm:justify-between"><div><span class="text-sm text-slate-400">Total do pedido</span><strong class="mt-1 block text-3xl">R$ <?php echo number_format($total, 2, ',', '.'); ?></strong></div><a href="index.php?pg=checkout" class="inline-flex items-center justify-center rounded-md bg-blue-500 px-6 py-3 text-sm font-extrabold uppercase tracking-wide transition hover:bg-blue-400">Finalizar compra</a></div>
        </div>
    <?php endif; ?>
</div>
