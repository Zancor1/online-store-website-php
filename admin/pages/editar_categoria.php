...
<div class="mb-8">
    ...
    <p class="text-gray-500 text-sm">Modifique as credenciais de @<?php echo $categoria['id']; ?>.</p>
</div>
...
<div class="bg-white border border-gray-100 rounded-3xl p-8 max-w-xl shadow-sm">
    ...
    <form action="index.php?pg=editar_categoria&id=<?php echo $categoria['id']; ?>" method="POST" class="space-y-6">
        
        <div class="flex flex-col gap-2">
            <label class="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">Novo nome da Categoria</label>
            <input type="text" name="nome_categoria" value="<?php echo $categoria['nome']; ?>" required class="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm focus:outline-none focus:border-indigo-600 focus:bg-white transition">
        </div>
