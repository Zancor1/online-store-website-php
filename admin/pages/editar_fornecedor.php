<?php
    ...
    $fornecedor = $id ? buscarFornecedoresPorId($id) : null;
...
<div class="mb-8">
    ...
    <p class="text-gray-500 text-sm">Editar os dados de <strong><?php echo htmlspecialchars($fornecedor['nome_fornecedor'] ?? ''); ?></strong>.</p>
</div>
...
<div class="bg-white border border-gray-100 rounded-3xl p-8 max-w-xl shadow-sm">
    ...
    <form action="index.php?pg=editar_fornecedor&id=<?php echo (int) $id; ?>" method="POST" class="space-y-4">
        <?php echo csrfCampo(); ?>
        <div class="flex flex-col gap-2">
            <label class="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">Nome Completo</label>
            <input type="text" name="nome_fornecedor" value="<?php echo htmlspecialchars($fornecedor['nome_fornecedor'] ?? ''); ?>" required class="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm focus:outline-none focus:border-indigo-600 focus:bg-white transition">
        </div>
        <div class="flex flex-col gap-2">
            <label class="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">CNPJ</label>
            <input type="text" name="cnpj_fornecedor" value="<?php echo htmlspecialchars($fornecedor['cnpj_fornecedor'] ?? ''); ?>" required class="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm focus:outline-none focus:border-indigo-600 focus:bg-white transition">
        </div>
        <div class="flex flex-col gap-2">
            <label class="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">Telefone</label>
            <input type="text" name="telefone_fornecedor" value="<?php echo htmlspecialchars($fornecedor['telefone_fornecedor'] ?? ''); ?>" required class="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm focus:outline-none focus:border-indigo-600 focus:bg-white transition">
        </div>  
        <div class="flex flex-col gap-2">
            <label class="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">CEP</label>
            <input type="text" name="cep_fornecedor" value="<?php echo htmlspecialchars($fornecedor['cep_fornecedor'] ?? ''); ?>" required class="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm focus:outline-none focus:border-indigo-600 focus:bg-white transition">
        </div>
        <div class="flex flex-col gap-2">
            <label class="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">Rua</label>
            <input type="text" name="rua_fornecedor" value="<?php echo htmlspecialchars($fornecedor['rua_fornecedor'] ?? ''); ?>" required class="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm focus:outline-none focus:border-indigo-600 focus:bg-white transition">
        </div>

        <div class="flex flex-col gap-2">
            <label class="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">Número</label>
            <input type="text" name="numero_fornecedor" value="<?php echo htmlspecialchars($fornecedor['numero_fornecedor'] ?? ''); ?>" required class="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm focus:outline-none focus:border-indigo-600 focus:bg-white transition">
        </div>

        <div class="flex flex-col gap-2">
            <label class="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">Bairro</label>
            <input type="text" name="bairro_fornecedor" value="<?php echo htmlspecialchars($fornecedor['bairro_fornecedor'] ?? ''); ?>" required class="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm focus:outline-none focus:border-indigo-600 focus:bg-white transition">
        </div>
        <div class="flex flex-col gap-2">
            <label class="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">Cidade</label>
            <input type="text" name="cidade_fornecedor" value="<?php echo htmlspecialchars($fornecedor['cidade_fornecedor'] ?? ''); ?>" required class="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm focus:outline-none focus:border-indigo-600 focus:bg-white transition">
        </div>
